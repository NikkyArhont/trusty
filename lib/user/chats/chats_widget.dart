    ...
    try {
      ...
      for (final chatId in _selectedChatIds) {
        batch.update(
          ...
          {'hiddenForClientAt': FieldValue.serverTimestamp()},
        );
      }
      await batch.commit();
      if (mounted) _cancelSelection();
    } catch (error) {
      debugPrint('Client chats hide failed: $error');
      if (mounted) {
          ...
          children: [
            ...
            Align(
              ...
              child: wrapWithModel(
                ...
                child: MenuWidget(currentPage: Menu.chats),
              ),
            ),
          ],
        ),
...
class _ChatListState extends State<_ChatList> {
  ...
  Future<void> _refresh() async {
    ...
    try {
      ...
      debugPrint('Client chat list refresh failed: $error');
    }
  }
      ...
      child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            debugPrint('Client chat list failed to load: ${snapshot.error}');
          ...
          final chats =
              snapshot.data!.docs.where((chat) {
                ...
                final hiddenAt = data['hiddenForClientAt'];
                final isHidden =
                    hiddenAt != null &&
